In this step, you'll define the core identity of your data product, which determines how all objects are named and tagged. This step captures:

1. **Data Product Name** (`data_product_name`) — The unique identifier for this data product (e.g., `sales_analytics`, `customer_360`)
2. **Domain** (`data_product_domain`) — The business unit that owns this data product (from Platform Foundation's `domain_list`)
3. **Environment** (`data_product_environment`) — The SDLC stage for this deployment (from Platform Foundation's `environment_list`)
4. **Description** (`data_product_description`) — A brief description of the data product's purpose

These values are used immediately in subsequent steps:
- **Create Databases**: Database names are constructed using these components
- **Create Schemas**: Schemas inherit tagging from the data product identity
- **Create Data Product Roles**: Role names include the data product name for clarity

All objects created in this workflow will be tagged with DOMAIN, ENVIRONMENT, and DATAPRODUCT for FinOps cost allocation.

**Account Context:** This step should be executed from the target account identified in the Select Target Account step.
...
- **Discoverability**: Clear naming helps users find and understand data assets

Think of this as naming a new project — the name you choose will appear on every document, meeting invite, and deliverable associated with it.
...
Your data product name is combined with domain, environment, and other components based on the naming conventions established in Platform Foundation.
