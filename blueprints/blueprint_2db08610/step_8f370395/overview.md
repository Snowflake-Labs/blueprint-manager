In this step, you'll execute SQL to assign the resource monitor to your data product's warehouses. This step produces:

1. **Monitor Assignments** — Each warehouse is connected to the resource monitor via `ALTER WAREHOUSE ... SET RESOURCE_MONITOR`

Once assigned, the monitor begins tracking usage immediately:
- Credit consumption from all assigned warehouses is aggregated
- Threshold percentages are calculated against the total quota
- Actions (notifications, suspensions) trigger when thresholds are reached

**Shared Quota Model**: By default, all data product warehouses share a single monitor's quota. If you need independent limits per warehouse, you would create separate monitors (not covered in this workflow).

This is the step where cost control becomes active. Before assignment, the monitor exists but isn't tracking anything.

**Account Context:** Execute this SQL from the target account with ACCOUNTADMIN role.

## **Why is this important?**

A resource monitor only works after assignment:
- **Tracking Starts**: Usage counting begins immediately
- **Limits Enforced**: Thresholds trigger configured actions
- **Aggregated Usage**: All assigned warehouses share the quota

Think of this as installing the circuit breaker — you've built it, now you're connecting it to the power lines.

## **Prerequisites**

- Resource monitor created in the Create Resource Monitors step
- Warehouses created in the Compute Configuration task

## **Key Concepts**

**Shared vs Individual Monitors**
- **Shared**: One monitor for all data product warehouses (recommended)
  - Total credits shared across warehouses
  - Simpler management
  
- **Individual**: Separate monitor per warehouse
  - Independent quotas
  - More complex to manage

**Usage Aggregation**
When multiple warehouses share a monitor:
- All usage counts toward the same quota
- Heavy use on one warehouse impacts others
- Good for data products with variable workloads

**Changing Assignments**
Warehouses can be reassigned to different monitors at any time:
- Previous monitor stops tracking
- New monitor starts tracking
- No gap in coverage

**More Information:**
* [Assign Warehouses to Resource Monitors](https://docs.snowflake.com/en/user-guide/resource-monitors#assigning-warehouses-to-a-resource-monitor) — Configuration guide
